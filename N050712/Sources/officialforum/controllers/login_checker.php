<?php
    ...
    //if $require_login was set and value is 'true'
        ...
        if(!isset($_SESSION['access_level'])){
            header("Location: {$home_url}controllers/index.php?action=please_login");
        }
    }
     
    // if it was the 'login' or 'register' or 'sign up' page but the customer was already logged in
    else if(isset($page_title) && ($page_title=="Đăng nhập" || $page_title=="Đăng ký")){
        // if user not yet logged in, redirect to login page
        if(isset($_SESSION['access_level']) && $_SESSION['access_level']=="Customer"){
            header("Location: {$home_url}manager/index.php?action=already_logged_in");
        }
    }
